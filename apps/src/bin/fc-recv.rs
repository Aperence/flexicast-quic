...
async fn main() {
    ...
    let current_fc_idx = args.idx_fc_chan;
    ...
    // Whether the client must leave the flexicast socket.
    ...
    let out_filename = url.path_segments().unwrap().last().unwrap();
    ...
    loop {
        ...
        if let Some(mc_socket) = mc_socket_opt.as_mut() {
            'mc_read: loop {
                ...
                debug!("Recv from socket flexicast processed");
            }
        }
        ...
        if conn.get_flexicast_attributes().is_some() {
            ...
            if let Some(flexicast) = conn.get_flexicast_attributes() {
                if flexicast.get_mc_role() ==
                    ...
                    must_leave_mc_sock
                {
                    info!("Leave the flexicast socket!");
        ...
        'streams: for stream_id in conn.readable() {
            ...
            while let Ok((read, fin)) = conn.stream_recv(stream_id, &mut buf[..])
            {
                ...
                let msg = FileTransferRecvMsg::Data(buf[..read].to_vec());
...
fn get_config(flexicast: bool) -> quiche::Config {
