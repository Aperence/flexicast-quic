//! Handles the HTTP/3 protocol for Flexicast QUIC.
...
impl Http3Server {
    ...
    pub fn handle_request(
        ...
        h3_conn: Option<&mut H3Conn>, _fc_chan_idx: usize,
    ) -> Result<Self> {
        let mut method = None;
        let mut path = vec![];
        ...
        if h3_resp.send_h3_headers {
            ...
            match h3_conn_to_use.send_response(
                conn,
                stream_id,
    ...
    pub fn start_request_on_fc_source(
        self, fh3_conn: &mut H3Conn, fc_chan: &mut FlexicastChannelSource,
        fh3_back: &mut H3Conn,
    ) -> Result<Self> {
        ...
        FlexicastChannelSource::advance(
            &mut fc_chan.channel,
        ...
        if let quiche::h3::Event::Headers { list, .. } = event {
            ...
            // Set the response as active because the flexicast server does not
            // wait.
