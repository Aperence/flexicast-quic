...
use crate::flexicast::reliable::ReliableFlexicastConnection;
...
pub(crate) struct FcRecovery {
    ...
    /// Packet number space identifier for this recovery structure.
    pub(crate) space_id: u64,
}

impl FcRecovery {
    /// New flexicast recovery.
    pub fn new(space_id: u64, is_fc_source: bool) -> Self {
        Self {
            is_fc_source,
    ...
    /// Insert a new acknowledged packet number.
    pub fn new_ack(&mut self, pkt_num: SpacedPktNum) {
        if pkt_num.0 == self.space_id as u32 {
            self.fc_new_ack_pn.push(pkt_num.1);
        }
    }

    /// Insert a new lost packet number.
    pub fn new_lost(&mut self, pkt_num: SpacedPktNum) {
        if pkt_num.0 == self.space_id as u32 {
            self.fc_new_lost_pn.push(pkt_num.1);
        }
    }
}
...
impl FlexicastRecovery for crate::recovery::Recovery {
    fn mc_data_timeout(
...
/// the multicast extension of QUIC.
pub trait ReliableFlexicastRecovery {
...
impl ReliableFlexicastRecovery for crate::recovery::Recovery {
    fn deleguate_stream(
        &mut self, uc: &mut Connection, space_id: u32,
        local_streams: &mut StreamMap, mc_ack: &mut McAck,
        retr_kind: FcUnicastRetransmission,
        ...
        let lost_iter = self.sent[Epoch::Application]
            ...
            .filter(|p| {
                (p.time_lost.is_some() ||
                    retr_kind == FcUnicastRetransmission::FullRetransmit ||
                    recv_ack_rangeset.contains(&p.pkt_num.1)) &&
                    p.pkt_num.0 == space_id
            });
        ...
        'per_packet: for packet in lost_iter {
            debug!(
                "This is a packet that is lost: {:?} with frames: {:?} and is lost={:?} is_acked={:?}", 
...
impl Recovery {
    ...
    pub fn fc_get_delegated_stream(
        ...
        for packet in lost_iter {
            debug!(
                "Lost packet: {:?} with frames: {:?}",
                packet.pkt_num, packet.frames
            );

            // Indicate that this packet was delegated through unicast
            // retransmission.
            // Do not say this if this is an early retransmit because the packet
            // may not be considered as lost.
            if !full_retransmit {
                packet.is_fc_delegated = true;
            }
...
mod tests {
    ...
    impl Recovery {
        /// Returns the set of packets currently in the sending queue.
        pub fn test_fc_empty_sending_queue(&self) -> Vec<&Sent> {
            self.sent[Epoch::Application].iter().collect()
        }
    }
