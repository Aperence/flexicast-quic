...
impl Connection {
    ...
    pub fn fc_change_channel(
        ...
        flexicast.set_mc_space_id(new_fc_space_id);

        flexicast.mc_leave_on_timeout = leave_on_timeout;
...
pub mod testing {
    ...
    /// Extension of the [`crate::flexicast::testing::MulticastPipe`].
    ...
    /// [`crate::flexicast::testing::MulticastPipe`] now only contains the
        ...
        pub fn new_defaults(keylog_filename: &str, nb: u8) -> Result<Self> {
            let fc_configs = (0..nb)
                .map(|idx| FcConfig {
                    ...
                    probe_mc_path: true,
                    authentication:
                        crate::flexicast::authentication::McAuthType::None,
                    ..FcConfig::default()
                })
                .collect();
...
mod tests {
    ...
    fn test_change_fc_chan_space_id() {
        ...
        // The flexicast space id is 1 (i.e., this is the first path created)...
        let (pipe, ..) = &mut mfc_pipe.fc_pipes[1].unicast_pipes[0];
        // ... for the client...
        let mc_space_id = pipe.client.flexicast.as_ref().unwrap().mc_space_id;
        assert_eq!(mc_space_id, Some(1));

        // ... and the server.
        let mc_space_id = pipe.server.flexicast.as_ref().unwrap().mc_space_id;
        assert_eq!(mc_space_id, Some(1));
        ...
        // The flexicast space id changed from 1 to 2 (because new path).
    ...
    fn test_change_fc_chan_single_rtt() {
        ...
        let fc_chan_id = pipe.client.flexicast.as_ref().unwrap().mc_announce_data
            [to]
            .channel_id
            .to_owned();
        ...
        let mc_announce =
            pipe.client.flexicast.as_ref().unwrap().mc_announce_data[to].clone();
        let server_addr =
            SocketAddr::new(IpAddr::V4(mc_announce.source_ip.into()), 4567);
        let client_addr = SocketAddr::new(
            IpAddr::V4(mc_announce.group_ip.into()),
        ...
        // The server adds the connection IDs of the flexicast channel.
        let mut scid = [0; 16];
        let random = SystemRandom::new();
        random.fill(&mut scid[..]).unwrap();
        let scid = ConnectionId::from_ref(&scid);
