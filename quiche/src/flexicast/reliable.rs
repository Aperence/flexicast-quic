...
use crate::recovery::multicast::FcRecovery;
use crate::recovery::multicast::ReliableFlexicastRecovery;
...
pub trait ReliableFlexicastConnection {
    ...
    /// server.
    ///
    /// Returns a [`crate::Error::Flexicast`] with
    /// [`crate::multicast::FcError::McInvalidRole`] if this is not a
    /// client.
    /// Returns a [`crate::Error::Flexicast`] with
    /// [`crate::multicast::FcError::McReliableDisabled`] if reliable
...
impl ReliableFlexicastConnection for Connection {
    fn rmc_timeout(&self, now: time::Instant) -> Option<time::Duration> {
    ...
    fn rmc_set_next_timeout(
        ...
        if let Some(multicast) = self.multicast.as_mut() {
            ...
            if let ReliableMc::Client(ref mut rmc) = multicast.mc_reliable {
                ...
                Err(Error::Flexicast(FcError::McReliableDisabled))
            }
        } else {
    ...
    fn rmc_should_send_positive_ack(&self) -> Result<bool> {
        self.multicast
            ...
            .mc_reliable
            .client()
            .ok_or(Error::Flexicast(FcError::McInvalidRole(McRole::Undefined)))
...
impl Connection {
    ...
    pub fn fc_set_recovery_state(&mut self) -> Result<()> {
        if let Some(multicast) = self.multicast.as_ref() {
            ...
            if let Some(space_id) = multicast.get_mc_space_id() {
                ...
                p.recovery.fc_recovery = Some(FcRecovery::new(space_id as u64, true));
    ...
    pub fn fc_bytes_in_flight(&self) -> Option<bool> {
        fc_chan_idx!(self.multicast.as_ref()?)
            .ok()
            .map(|idx| {
                self.paths
                    .get(idx)
...
pub mod testing {
    ...
    impl MulticastPipe {
        ...
        pub fn source_deleguates_streams_direct(
            ...
            ucs.map(|uc| mc.rmc_deleguate_streams(uc, expired, retr_kind.clone()))
                .collect()
        }
...
mod tests {
    ...
    fn test_fc_quic_reliability_with_mc_ack() {
        ...
        assert_eq!(fc_pipe.client_rmc_timeout(now, &random), Ok(()));
        let fc_path = fc_pipe.mc_channel.channel.paths.get(1).unwrap();
        assert_eq!(fc_path.recovery.acked[Epoch::Application].len(), nb_ack + 1);
        ...
        fc_pipe
            .source_deleguates_streams_direct(
                now,
                FcUnicastRetransmission::Delegates,
            )
            .unwrap();
    ...
    fn test_fc_quic_reliability_fcf_failing() {
        ...
        fc_pipe.server_control_to_mc_source(now).unwrap();
        for (uc_pipe, ..) in fc_pipe.unicast_pipes.iter_mut() {
        ...
        fc_pipe
            .mc_channel
            .channel
            .rmc_deleguate_streams(uc, now, FcUnicastRetransmission::Delegates)
            .unwrap();
    ...
    fn test_fc_quic_reliability_recv_retransmit() {
        ...
        let rmc_server = fc_pipe.unicast_pipes[0].0.server.multicast.as_ref().unwrap().rmc_get().server().unwrap();
        let mut ack_pn = RangeSet::default();
        ack_pn.insert(4..5);
        assert_eq!(rmc_server.new_ack_pn_fc, ack_pn);
        fc_pipe.server_control_to_mc_source(now).unwrap();
        ...
        fc_pipe.mc_channel.channel.rmc_deleguate_streams(uc, now, retr_kind).unwrap();
        ...
        fc_pipe.mc_channel.channel.on_mc_timeout(now).unwrap();
        let uc = &mut fc_pipe.unicast_pipes[1].0.server;
        let retr_kind = FcUnicastRetransmission::Delegates;
        fc_pipe.mc_channel.channel.rmc_deleguate_streams(uc, now, retr_kind).unwrap();
        fc_pipe.unicast_pipes[1].0.advance().unwrap();
        ...
        let sent = p.recovery.test_fc_empty_sending_queue();
