...
impl FcSendBuf {
    ...
    pub fn write_at_offset(
        ...
        if let Some(send_buf) = self.fc_send_buf.as_mut() {
            ...
            return Err(Error::Flexicast(FcError::FcStreamRotation));
        }
    }
    ...
    pub fn emit(&mut self, out: &mut [u8]) -> Result<(usize, bool)> {
        if let Some(send_buf) = self.fc_send_buf.as_mut() {
            ...
            Err(Error::Flexicast(FcError::FcStreamRotation))
        }
    }
...
mod tests {
    ...
    fn test_fc_send_buf_uc() {
        ...
        stream.send.fc_set_fin_off(10 + first.len() as u64);
